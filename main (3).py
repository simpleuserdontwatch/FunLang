...
def findifend(lines,c):
  for e,i in enumerate(lines[c::]):
    if i == 'WHENEND':
      return e
  error('no when end?',c,"")
def runline(line,canuseos,canusestdio,canusewait,canuserandom,env,c,code,safe):
  parsed = parse(line)
  if not parsed:
    return
  if parsed[0] == 'SAY':
    ...
    while run:
      ...
      emulate2(code[loopstart+2:loopend],canuseos,canusestdio,canusewait,canuserandom,env,safe)
...
def emulate(code,safe=False):
  ...
  for c,line in enumerate(code[1:]): # slice one line cause headers was removed😢
    if skiplines < 1:
      ret = runline(line,canuseos,canusestdio,canusewait,canuserandom,env,c,code,safe)
      if ret:
        if ret[0] == 'SKIP':
          skiplines += ret[1]
        else:
...
def emulate2(code,canuseos,canusestdio,canusewait,canuserandom,env,safe):
  for c,line in enumerate(code):
    runline(line,canuseos,canusestdio,canusewait,canuserandom,env,c,code,safe)
